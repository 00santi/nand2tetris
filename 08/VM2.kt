...
fun main(args: Array<String>) {
    ...
    val input_path = File(args[0])
    val files = if (input_path.isDirectory) {
        input_path.listFiles { f -> f.extension == "vm" }?.toList() ?: emptyList()
    } else {
        listOf(input_path)
    }
    ...
    if (files.size > 1) sysinit()

    for (f in files) {
        println("\n-----${f.name}-----")
        current_filename = f.name.substringBeforeLast('.')
        val lines = f.readLines().map { clean_line(it) }.filter { it.isNotEmpty() }
        parse(lines)
    }

    val output_name = if (files.size > 1) input_path.name else files[0].nameWithoutExtension
    File("$output_name.asm").writeText(output.joinToString("\n"))
}
...
val output = ArrayList<String>(200)
fun parse(lines: List<String>) {
    for (line in lines) {
        ...
        when (tokens[0]) {
            ...
            "function" -> function(tokens[1], tokens[2].toInt())
            "call" -> call(tokens[1], tokens[2])
            "return" -> return_()
        }
    }
}
...
var call_counter = 0
fun call(f: String, n: String) {
    val return_label = "RETURN_LABEL${call_counter}"
    output += listOf("@$return_label",
        "D=A",
        "@SP",
        "A=M",
        "M=D",
    ...
    for(base in listOf("@LCL", "@ARG", "@THIS", "@THAT")) {
        output += listOf(base,
            "D=M",
            "@SP",
            "A=M",
...
fun return_() {
    ...
    for(base in listOf("@THAT", "@THIS", "@ARG", "@LCL")) {
        output += listOf(
            "@R13",
            "AM=M-1",
            "D=M",
...
fun label(X: String) {
    val temp = if(current_function_name.isEmpty()) X else "$current_function_name$$X"
    output += "($temp)"
}
fun goto(X: String) {
    val temp = if(current_function_name.isEmpty()) X else "$current_function_name$$X"
    output += listOf("(@$temp)", "0;JMP")
}
fun if_goto(X: String) {
    val temp = if(current_function_name.isEmpty()) X else "$current_function_name$$X"
    output += listOf(
        "@SP",
        "AM=M-1",
